...
def export_conversation():
    ...
    for msg in st.session_state.messages:
        if msg["role"] == "user":
            ...
            html_content += (
                f'<div class="answer"><strong>Analysis:</strong><br>{content}</div>'
            )
            if "figure" in msg:
...
with st.sidebar:
    ...
    if uploaded_file:
        try:
            ...
            with st.expander("Data Summary"):
                ...
                with col2:
                    st.metric(
                        "Memory Usage", f"{df.memory_usage().sum() / 1024:.1f} KB"
                    )
    ...
    if st.session_state.messages:
        ...
        if st.sidebar.button("Generate Report"):
            ...
            st.sidebar.info(
                "💡 Tip: Open the HTML file and print to PDF for best results"
            )
                        ...
                        for i in range(1, len(code_blocks)):
                            ...
                            try:
                                ...
                                if "NameError" in str(e):
                                    st.info(
                                        "This might mean a column name is misspelled or doesn't exist."
                                    )
                                    ...
                                    st.info(
                                        "The specified column might not exist in your dataset."
                                    )
                                else:
                                    st.info(
